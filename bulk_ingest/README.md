...
* We suspect the anomalous behavior could also be due to the fact that network latency effects could have been introduced, since the insertions were all performed through a Python script on a remote AWS hosted database. Alternatively, the number of rows may not have been considerably large to clearly and consistently observe effects related to indexing.  
...
* The results confirm our Hypothesis for upto 3 indexes (the throughput decreases as the number of indexes increases) and surprisingly for 4 indexes we observe some discrepancy (throughput has increased compared to 3 indexes).
* The throughput is reduced by 4% in the presence of three indexes as compared to 1.6% with 4 indexes.
...
* Another way to speed up bulk inserts could be to drop indexes temporarily while loading large amounts of data and then rebuilding the indexes, an approach often used in data-warehousing scenarios (Winand, n.d.).
